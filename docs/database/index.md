...
[Início](../../README.md) | 🛢️ Banco de dados
...
#### Colunas de Status do Projeto (project_status_columns)

```sql
-- TABELA: project_status_columns
-- Descrição: Coluna de status que uma tarefa pode ter dentro de um projeto
-- Relacionamentos:
--   - project_id → projects(id)
--   - created_by/updated_by/disabled_by → users(id)
-- Indexes:
...
-- Relacionamentos:
--   - project_id → projects(id)
--   - column_status_id → task_column_statuses(id)
--   - created_by/updated_by/disabled_by → users(id)
-- Indexes:
--   - project_id, column_status_id, created_by, priority, disabled_at

CREATE TABLE tasks (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    external_id CHAR(36) NOT NULL UNIQUE,
    project_id BIGINT UNSIGNED NOT NULL,
    column_status_id BIGINT UNSIGNED NOT NULL,
